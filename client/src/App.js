...
import {check,fetchComponents,getInterface,fetchBlocks, changeSite} from "./http/API";
const App = observer(() => {
    ...
    useEffect(()=>{
        if(inface.interface.id > -1){}else{
            getInterface().then(data=>{
                ...
                //link.href = "https://stroidiskont.com:14443/Логотип премиум_веб-03.png";
        ...
        if(!inface.blocks.length || !inface.pages.length || !inface.images.length){
            fetchBlocks().then(data=>{
                ...
                inface.setImages(data.images);
                inface.setPatterns(data.patterns);
            })
        }
        if(user.role === 0){
    ...
    /*if (document.location.pathname.includes('/image/')) {
        ...
        image.src = process.env.REACT_APP_API_URL + decodeURI(document.location.pathname.replace('image', '').replace(/\//g, ''))
        document.body.innerHTML = ''
        async function getImage(){
            let f = await fetch(process.env.REACT_APP_API_URL + decodeURI(document.location.pathname.replace('image', '').replace(/\//g, '')));
            let blob = await f.blob()
            return blob
            URL.createObjectURL(blob)
            //document.body.innerHTML = blob
        }
        getImage();
        //document.appendChild(image);
        return
    } else {
        return <img src='https://stroidiskont.com/image/изображение_2023-04-27_225228498.png'/>
    }*/
    ...
    return (
        <BrowserRouter>
            ...
            <svg display='none' preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <symbol id='list'>
                    <path d="M223.999,128.00037a8.00039,8.00039,0,0,1-8,8h-176a8,8,0,0,1,0-16h176A8.00039,8.00039,0,0,1,223.999,128.00037Zm-184-56h176a8,8,0,0,0,0-16h-176a8,8,0,1,0,0,16Zm176,112h-176a8,8,0,0,0,0,16h176a8,8,0,0,0,0-16Z"/>
                </symbol>
                <symbol >
