...
export const order = async (name,phone,price,priceList,delivery,address) => {
    let form = new FormData()
    form.append('name',name);
    form.append('phone',phone);
    form.append('price',price);
    form.append('priceList',priceList);
    form.append('delivery',delivery);
    form.append('address',address);
    const {data} = await $host.post('api/mail/order', form)
    return data
}
export const sendQuiz = async (form) => {
    const {data} = await $host.post('api/mail/quiz', form)
    return data
}
export const mail = async (proc,form) => {
    const {data} = await $host.post('api/mail/mail/' + proc, form)
    return data
}
