...
const MainPage = observer(() => {
    /*if (document.location.pathname.includes('object')) {
        return
    }*/
    ...
    const [delivery, setDelivery] = useState('Не выбрано');
    ...
    let [page,setPage] = useState(inface.pages.filter(page=>document.location.pathname.includes('object') ? page.path === '' : page.path === document.location.pathname.replace(ADMIN_ROUTE,'').replace('/',''))[0])
    //let [page,setPage] = useState(inface.pages.filter(page=>page.path === 'onix-mramor')[0])
    //console.log(page)
    useEffect(()=>{
        let page1;
        page1 = inface.pages.filter(page=>document.location.pathname.includes('object') ? page.path === '' : page.path === document.location.pathname.replace(ADMIN_ROUTE,'').replace('/',''))[0];
        //page1 = inface.pages.filter(page=>page.path === 'onix-mramor')[0];
        if (page1) {
            ...
            if(JSON.parse(page1.obj || '{}').js && (!user.role > 0 || !document.location.pathname.includes(ADMIN_ROUTE))){
                try{window.eval(JSON.parse(page1.obj || '{}').js)}catch(err){console.error('Ошибка во время исполнения кода: \n\n',err)}
            }
            setPage(page1);

            if (document.location.pathname.includes('object')) {
                inface.setCurrProduct(inface.blocks.filter(d=>d.id === parseInt(document.location.pathname.replace('object', '').replace(/\//g, '')))[0]);
                document.getElementById('theOnlyOneBody').style.overflow = 'hidden';
            } else {
                inface.setCurrProduct(null);
                document.getElementById('theOnlyOneBody').style.overflow = 'auto';
            }
        }

    },[document.location.pathname, inface.blocks])
    function checkBtn(){
    ...
    useEffect(()=>{
        if(loadCount <= inface.blocks.length && inface.blocks.filter(bl=>bl.page === page.id)[0]?inface.show:true){
            const loadTimer = setTimeout(()=>{setLoadCount(loadCount + 1)},400)
            return () => clearTimeout(loadTimer)
        }
    },[loadCount,inface.show])
    useEffect(()=>{
        if(inface.blocks.filter(bl=>bl.page === page)[0]?inface.show:true){
    ...
    },[document.getElementById('root').offsetHeight,document.querySelectorAll('.FDAStart').length,(inface.blocks.filter(bl=>page && bl.page === page.id && bl.type==='post-header')[0]?inface.show:true)])
        ...
        document.addEventListener('click',e=>{
            if(inface.blockType){
                ...
                if((obj2.type==='Контейнер' || obj2.type==='Список товаров') || (inface.moveblock === '' || inface.moveblock.includes('page'))){
                    ...
                    addBlock(inface.blockType,(inface.moveblock === '' || inface.moveblock.includes('page'))?1:inface.blocks.filter(d=>d.parent === inface.moveblock).length + 1,inface.moveblock === '' || inface.moveblock.includes('page')?'page'+page.id:inface.moveblock,JSON.stringify(obj3)).then(d=>inface.setBlocks(d))
    ...
    return(
        <div style={{width:'100%',color:inface.interface.color,background:inface.interface.background,fontSize:inface.width > 350?inface.width > 600?'16px':'12px':'11px',position:'relative',}}>
            {user.role > 0 && document.location.pathname.includes(ADMIN_ROUTE) && <div id={'blockSettings'} style={{overflowY:'auto',boxShadow:'-5px 0px 50px #C7C7C799',position:'fixed',width:'300px',zIndex:'1009',height:inface.height,top:'0px',right:0,opacity:0,display:'none',transitionDuration:"0.5s",fontWeight:'bold',fontSize:"0.8em",background:'white',color:'black',padding:'65px 10px',}}>
                ...
                <div style={{marginTop:'10px',cursor:'pointer',display:'inline-block',color:'red'}} onClick={()=>{if(window.confirm('Вы действительно хотите безвозвратно удалить этот блок?')){deleteBlock(inface.currSettings.id).then(d=>{inface.setBlocks(d)}).catch(d=>alert(d));inface.setCurrSettings(null);document.getElementById('blockSettings').style.display = 'none';document.getElementById('blockSettings').style.opacity = 0;inface.setMoveblock(inface.currSettings.parent);}}}>Удалить</div>
            ...
            <div style={{position:'relative',display:'grid',width:'100%',justifyItems:'center'}}>
                {user.role > 3 && page && document.location.pathname.includes(ADMIN_ROUTE) &&<div style={{zIndex:'1009',position:'relative',display:'grid',justifyItems:'center',gridAutoFlow:'column',alignItems:'center',fontWeight:'bold',width:'100%',textAlign:'center',padding:'5px 15px',color:inface.aback,background:inface.acolor,}}>
                    <div>
            ...
            {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE) && inface.currImageProp &&<div style={{overflowY:'auto',width:inface.width * 0.8 + 'px',height:inface.height + 'px',position:'fixed',top:'0px',left:'0',display:'block',zIndex:'1011',padding:'0% '+(inface.width * 0.1) + 'px',}}>
                ...
                <div style={{overflowY:'hidden',margin:'7% 0',justifyItems:'center',gridGap:'1em',display:'grid',gridTemplateRows:'min-content min-content 1fr',background:inface.aback,zIndex:'2',width:'calc(100% - 60px)',minHeight:'100%',padding:'30px',borderRadius:'15px',position:'relative',alignItems:'start'}}>
                    <h3>Файлы</h3>
                    ...
                    <div style={{position:'relative',width:'100%',gridGap:'1em',display:'grid',gridTemplateColumns:'repeat(4,1fr)'}}>
                        {inface.images.slice().reverse().map((d,key)=><div key={'uploadImg'+d.id} style={{display:'grid',gridGap:'1em',width:'100%',placeItems:'center',position:'relative'}}>
                            ...
                            {d.path.includes('.mp4') ?
                                <h2 onClick={()=>{let val = null;if(inface.currImageProp.toLowerCase().includes('arr')){let arr1 = JSON.parse(JSON.parse(inface.currSettings.obj || '{}').imgArr || '[]');arr1.push(d.path);val = JSON.stringify(arr1)}else{val = d.path};setObjProperty(inface.currImageProp,val,inface.currSettings.id).then(data=>{inface.setBlocks(data);inface.setCurrSettings(data.filter(d=>d.id === inface.currSettings.id)[0]);inface.setCurrImageProp(null);document.getElementById('theOnlyOneBody').style.overflow = 'auto'})}} style={{cursor:'pointer'}}>Видео</h2>
                            :
            ...
            <div id={'productBG'} onClick={()=>{

                /*document.getElementById('theOnlyOneBody').style.overflow = 'auto';
                inface.setCurrProduct(null);*/
            }} style={{zIndex:'1012',width:'100%',height:document.documentElement.clientHeight + 'px',backgroundColor:'black',opacity:'0.5',position:'fixed',top:'0',display:inface.currProduct?'block':'none'}}/>
            <div id={'product'} onMouseUp={e=>{
                if(!document.getElementById('productCont').contains(e.target)){
                    /*document.getElementById('theOnlyOneBody').style.overflow = 'auto';
                    inface.setCurrProduct(null);
                    setProdCount(1);*/
                    if (user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)) {
                ...
                }}} style={{zIndex:'1013',display:inface.currProduct?'block':'none',position:'fixed',overflow:'auto',width:'100%',bottom:'0',height:document.documentElement.clientHeight + 'px',padding:'0px 0',}}>
            ...
            {inface.basket.length && cost > 0 ? <div style={{position:'fixed',cursor:'pointer',display:'grid',gridTemplateColumns:'1fr 1fr',bottom:'1em',left:inface.mobile ? '5%' : 'calc(50% - 300px - 1em)',background:JSON.parse(page.obj || '{}').color || '#F94F0D',color:'white',maxWidth:'90%',width:inface.mobile ? '90%' : '600px',padding:'0.7em 1em',fontSize:inface.mobile ? '1.3em' : '1.1em',fontWeight:'bold',borderRadius:'40px',}} onClick={()=>{
                document.getElementById('theOnlyOneBody').style.overflow = 'hidden';
                document.getElementById('make_order').style.display = 'grid';
            }}>
            ...
            <div id='make_order' style={{position:'fixed',width:'100%',height:inface.height + 'px',top:'0',display:'none',placeItems:'center',overflowY:'auto'}}>
                <div style={{background:'rgba(0,0,0,0.3)',width:'100%',height:'100%',gridColumn:'1/2',gridRow:'1/2',backdropFilter: 'blur(5px)'}} onClick={()=>{
                    document.getElementById('theOnlyOneBody').style.overflow = 'auto';
                    document.getElementById('make_order').style.display = 'none';
                }}></div>
                {inface.mobile ? <div style={{display:'grid',gridTemplateColumns:inface.mobile ? '1fr' : '3fr 1px 2fr',gridGap:'1em',background:'white',width:inface.mobile ? '100%' : '1100px',maxWidth:'100%',padding:'25px 25px',minHeight:'600px',borderRadius:'10px',gridColumn:'1/2',gridRow:'1/2',position:'relative',boxSizing:'border-box',margin:'250px 0',}}>
                    <div style={{fontSize:'1.4em',textAlign:'center',fontWeight:'bold',marginBottom:'30px'}}>Оформить заказ</div>
                    <div style={{display:'grid',}}>
                        <svg onClick={()=>{
                            document.getElementById('theOnlyOneBody').style.overflow = 'auto';
                            document.getElementById('make_order').style.display = 'none';
                        }} style={{cursor:'pointer',top:inface.mobile ? '20px' : '20px',width:'12px',height:'12px',right:inface.mobile ? '20px' : '20px',fill:'grey',overflow:'hidden',position:'absolute'}} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><use xlinkHref='#crossSVG' ></use></svg>
                        <div style={{textAlign:'center',fontWeight:'bold',marginBottom:'10px'}}>Выбранные товары:</div>
                        <div style={{maxHeight:inface.height * 0.7,overflowY:'auto',background:'#f1f1f1',padding:'5px 20px',boxSizing:'border-box'}}>
                            <table>
                                <tr>
                                    <td style={{paddingLeft:0,paddingRight:0}}><b>Товар</b></td>
                                    <td style={{width:'50px'}}><b>Цена</b></td>
                                    <td style={{paddingRight:0,paddingLeft:0}}><b>Кол-во</b></td>
                                </tr>
                                {inface.blocks.map(block => {
                                    if (inface.basket.includes(block.id)) {
                                        return <tr>
                                            <td style={{paddingLeft:0,paddingRight:0,fontSize:'0.9em'}}>- {JSON.parse(block.obj || '{}').title}</td>
                                            <td style={{width:'60px',fontSize:'0.9em'}}><b>{JSON.parse(block.obj || '{}').price.toString().replace(/\B(?=(?:\d{3})+(?!\d))/g, ' ')} ₽</b></td>
                                            <td style={{paddingLeft:0,paddingRight:0}}>
                                                <div style={{placeSelf:"center",display:'grid',gridTemplateColumns:'1fr 1fr 1fr',placeItems:'center',gridGap:'2px'}} className='noselect'>
                                                    <div onClick={()=>{if(inface.basket.filter(el=>el === block.id).length > 0){let arr1 = inface.basket;arr1.splice(arr1.indexOf(block.id), 1);inface.setBasket(arr1);}}} style={{float:'right',cursor:'pointer',padding:'6px 4px 7px 4px',fontWeight:'bold',fontSize:'1.3em',lineHeight:'1px',borderRadius:'50%',boxShadow:'0 1px 14px 1px rgb(0 0 0 / 10%)',background:'#c4c4c4'}}>-</div>
                                                    <div style={{}}>{inface.basket.filter(el=>el === block.id).length}</div>
                                                    <div onClick={()=>{let arr1 = inface.basket;arr1.push(block.id);inface.setBasket(arr1);}} style={{cursor:'pointer',padding:'8px 3px 6px 3px',fontWeight:'bold',fontSize:'1.3em',lineHeight:'0',borderRadius:'50%',boxShadow:'0 1px 14px 1px rgb(0 0 0 / 10%)',background:'#c4c4c4'}}>+</div>
                                                </div>
                                            </td>
                                        </tr>;
                                    }
                        ...
                        <span style={{marginTop:'20px',placeSelf:'center'}}>К оплате: <b>{cost.toString().replace(/\B(?=(?:\d{3})+(?!\d))/g, ' ')}</b> рублей</span>
                    </div>
                    <div>
                        <div>
                            Стоимость доставки по городу Чебоксары:<br/>-600 рублей до 100 кг;<br/>-700 рублей до 500 кг;<br/>-900 руб до 1,5 тонны одна грузовая машина.<br/>Более тяжелый груз обговаривается отдельно.
                            <br/><br/>
                            После оплаты Вашего заказа с Вами свяжется наш менеджер и уточнит все детали доставки или даст информацию по самовывозу.
                        </div>
                        <div style={{paddingTop:'20px',width:'100%',display:'grid'}}>
                            <b>Заполните поля:</b>
                            <div style={{display:'grid',gridTemplateColumns:inface.mobile ? '1fr' : '1fr 1fr',placeItems:'stretch',margin:'20px 0',gridGap:'1em'}}>
                                <input value={name} onChange={e=>{setName(e.target.value)}} placeholder="Ваше имя*" type="text" className="formInput"/>
                                <input value={phone} onChange={e=>{setPhone(e.target.value)}} placeholder="Номер телефона*" type="text" className="formInput"/>
                            </div>
                            <div style={{fontWeight:'bold'}}>
                                Выберите способ получения:
                                <select value={delivery} onChange={e=>setDelivery(e.target.value)} style={{marginLeft:'10px'}}>
                                    <option>Не выбрано</option>
                                    <option>Самовывоз</option>
                                    <option>Доставка</option>
                                </select>
                            </div>
                            {delivery === 'Доставка' && <div style={{display:'grid',gridAutoFlow:'row',gridGap:'4px',marginTop:'20px',gridTemplateColumns:'1fr 1fr'}}>
                                <input style={{minWidth:'120px'}} value={city} onChange={e=>{setCity(e.target.value)}} placeholder="Населённый пункт*" type="text" className="formInput"/>
                                <input style={{minWidth:'80px'}} value={street} onChange={e=>{setStreet(e.target.value)}} placeholder="Улица*" type="text" className="formInput"/>
                                <input style={{minWidth:'70px'}} value={building} onChange={e=>{setBuilding(e.target.value)}} placeholder="Строение*" type="text" className="formInput"/>
                                <input style={{minWidth:'70px'}} value={apartment} onChange={e=>{setApartment(e.target.value)}} placeholder="Квартира/офис*" type="text" className="formInput"/>
                            </div>}
                            {clicked && (name.length < 2 || phone.length < 6 || (delivery === 'Не выбрано' || delivery === 'Доставка' && (city.length < 2 || street.length < 2 || building.length === 0))) && <span style={{justifySelf:'center',marginTop:'10px',fontSize:'0.9em',color:'red',}}>{delivery === 'Не выбрано' ? 'Выберите способ доставки' : 'Заполните все поля'}</span>}
                            <button onClick={()=>{
                                setClicked(true);
                                if (!(name.length < 2 || phone.length < 6 || (delivery === 'Не выбрано' || delivery === 'Доставка' && (city.length < 2 || street.length < 2 || building.length === 0)))) {
                                    let priceList = '';
                                    inface.blocks.map(block => {
                                        if (inface.basket.includes(block.id)) {
                                            priceList += '- ' + JSON.parse(block.obj || '{}').title + ', ' + JSON.parse(block.obj || '{}').price + ' ₽ * <b>' + inface.basket.filter(el=>el === block.id).length + ' шт.</b><br/>'
                                        }
                                    });
                                    let address = city + ', ул. ' + street + ', строение ' + building + ', квартира/офис ' + apartment;
                                    order(name, phone, cost, priceList, delivery, address).then(data=>{

                                    });
                                    inface.setBasket([]);
                                    setCost(0);
                                }
                            }} style={{border:0,cursor:'pointer',borderRadius:'10px',background:clicked && (name.length < 2 || phone.length < 6 || (delivery === 'Не выбрано' || delivery === 'Доставка' && (city.length < 2 || street.length < 2 || building.length === 0))) ? 'grey' : '#F64E0DFF',color:'white',padding:'10px 70px',fontWeight:600,fontSize:'1.1em',marginTop:'40px',justifySelf:'center'}}>Купить</button>
                        </div>
                    </div>
                </div>
                :
                <div style={{display:'grid',gridTemplateColumns:'3fr 1px 2fr',gridGap:'1em',background:'white',width:'1100px',maxWidth:inface.width,padding:'25px 25px',minHeight:'600px',borderRadius:'10px',gridColumn:'1/2',gridRow:'1/2',position:'relative'}}>
                    <div>
                        <div style={{fontSize:'1.3em',textAlign:'center',fontWeight:'bold',marginBottom:'30px'}}>Оформить заказ</div>
                        <div>
                            Стоимость доставки по городу Чебоксары:<br/>-600 рублей до 100 кг;<br/>-700 рублей до 500 кг;<br/>-900 руб до 1,5 тонны одна грузовая машина.<br/>Более тяжелый груз обговаривается отдельно.
                            <br/><br/>
                            После оплаты Вашего заказа с Вами свяжется наш менеджер и уточнит все детали доставки или даст информацию по самовывозу.
                        ...
                        <div style={{paddingTop:'20px',width:'100%',display:'grid'}}>
                            <b>Заполните поля:</b>
                            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',placeItems:'stretch',margin:'20px 0',gridGap:'4em'}}>
                                <input value={name} onChange={e=>{setName(e.target.value)}} placeholder="Ваше имя*" type="text" className="formInput"/>
                                <input value={phone} onChange={e=>{setPhone(e.target.value)}} placeholder="Номер телефона*" type="text" className="formInput"/>
                            </div>
                            <div style={{fontWeight:'bold'}}>
                            ...
                            {delivery === 'Доставка' && <div style={{display:'grid',gridAutoFlow:'column',gridGap:'4px',marginTop:'20px'}}>
                                <input style={{minWidth:'150px'}} value={city} onChange={e=>{setCity(e.target.value)}} placeholder="Населённый пункт*" type="text" className="formInput"/>
                                <input style={{minWidth:'80px'}} value={street} onChange={e=>{setStreet(e.target.value)}} placeholder="Улица*" type="text" className="formInput"/>
                                <input style={{minWidth:'70px'}} value={building} onChange={e=>{setBuilding(e.target.value)}} placeholder="Строение*" type="text" className="formInput"/>
                                <input style={{minWidth:'100px'}} value={apartment} onChange={e=>{setApartment(e.target.value)}} placeholder="Квартира/офис*" type="text" className="formInput"/>
                            </div>}
                            {clicked && (name.length < 2 || phone.length < 6 || (delivery === 'Не выбрано' || delivery === 'Доставка' && (city.length < 2 || street.length < 2 || building.length === 0))) && <span style={{justifySelf:'center',marginTop:'10px',fontSize:'0.9em',color:'red',}}>{delivery === 'Не выбрано' ? 'Выберите способ доставки' : 'Заполните все поля'}</span>}
                            <button onClick={()=>{
                                setClicked(true);
                            ...
                            }} style={{border:0,cursor:'pointer',borderRadius:'10px',background:clicked && (name.length < 2 || phone.length < 6 || (delivery === 'Не выбрано' || delivery === 'Доставка' && (city.length < 2 || street.length < 2 || building.length === 0))) ? 'grey' : '#f64e0d',color:'white',padding:'10px 70px',fontWeight:600,fontSize:'1.1em',marginTop:'40px',justifySelf:'center'}}>Купить</button>
                        </div>
                    </div>
                    ...
                    <div style={{display:'grid',}}>
                        <svg onClick={()=>{
                            document.getElementById('theOnlyOneBody').style.overflow = 'auto';
                            document.getElementById('make_order').style.display = 'none';
                        }} style={{cursor:'pointer',top:inface.mobile ? '20px' : '20px',width:'20px',height:'20px',right:inface.mobile ? '20px' : '20px',fill:'grey',overflow:'hidden',position:'absolute'}} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><use xlinkHref='#crossSVG' ></use></svg>
                        <div style={{fontSize:'1.3em',textAlign:'center',fontWeight:'bold',marginBottom:'10px'}}>Выбранные товары:</div>
                        <div style={{maxHeight:inface.height * 0.7,overflowY:'auto',background:'#f1f1f1',padding:'5px 20px'}}>
                            <table>
                                <tr>
                                    <td style={{paddingLeft:0,paddingRight:0}}><b>Товар</b></td>
                                    <td style={{width:'50px'}}><b>Цена</b></td>
                                ...
                                {inface.blocks.map(block => {
                                    if (inface.basket.includes(block.id)) {
                                        return <tr>
                                            <td style={{paddingLeft:0,paddingRight:0,fontSize:'0.9em'}}>- {JSON.parse(block.obj || '{}').title}</td>
                                            <td style={{width:'55px'}}><b>{JSON.parse(block.obj || '{}').price} ₽</b></td>
                                            <td style={{paddingLeft:0,paddingRight:0}}>
                                                <div style={{placeSelf:"center",display:'grid',gridTemplateColumns:'1fr 1fr 1fr',placeItems:'center',gridGap:'2px'}} className='noselect'>
                                                    <div onClick={()=>{if(inface.basket.filter(el=>el === block.id).length > 0){let arr1 = inface.basket;arr1.splice(arr1.indexOf(block.id), 1);inface.setBasket(arr1);}}} style={{float:'right',cursor:'pointer',padding:inface.mobile ? '6px 4px 6px 4px' : '8px 7px 12px 7px',fontWeight:'bold',fontSize:'1.3em',lineHeight:'1px',borderRadius:'50%',boxShadow:'0 1px 14px 1px rgb(0 0 0 / 10%)',background:'#c4c4c4'}}>-</div>
                                                    <div style={{}}>{inface.basket.filter(el=>el === block.id).length}</div>
                                                    <div onClick={()=>{let arr1 = inface.basket;arr1.push(block.id);inface.setBasket(arr1);}} style={{cursor:'pointer',padding:inface.mobile ? '6px 3px 6px 3px' : '10px 5px 12px 5px',fontWeight:'bold',fontSize:'1.3em',lineHeight:'0',borderRadius:'50%',boxShadow:'0 1px 14px 1px rgb(0 0 0 / 10%)',background:'#c4c4c4'}}>+</div>
                                                </div>
                                            </td>
                                        </tr>;
                                    }
                        ...
                        <span style={{marginTop:'20px',placeSelf:'center'}}>К оплате: <b>{cost}</b> рублей</span>
                    </div>
                </div>
                }
            </div>
