...
const Distributor = observer((data) => {
    ...
    return(
        <div ref={block} id={'block'+data.id}
            onMouseOver={()=>{if(inface.moveblock !== 'block'+data.id && (!inface.moveblock.includes('block') || inface.blocks.filter(d=>d.id+'' === inface.moveblock.replace('block','')).length === 0 || (inface.moveblock === '' || (document.getElementById(inface.moveblock) && document.getElementById(inface.moveblock).offsetWidth > document.getElementById('block'+data.id).offsetWidth) || parseInt(inface.moveblock.replace('block','')) < data.id))){inface.setMoveblock('block'+data.id);}}}
            onMouseOut={e=>{let rect = document.getElementById('block'+data.id).getBoundingClientRect();if(inface.moveblock === 'block'+data.id && !data.parent.includes('page') && (e.pageX < rect.left || e.pageX > rect.right || e.pageY < rect.top || e.pageY > rect.bottom)){inface.setMoveblock(data.parent);}}} onClick={()=>{
                if (user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)) {
                    ...
                    if ((JSON.parse(data.obj || '{}').href || '').includes('http') || (JSON.parse(data.obj || '{}').href || '').includes('viber')) {
            ...
            {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE) &&<div style={{display:inface.moveblock === 'block' + data.id?'grid':'none',zIndex:'1000',transitionDuration:'0.35s',top:'5px',left:'5px',background:inface.acolor,padding:'10px 12px',borderRadius:"10px",position:'absolute',gridAutoFlow:'column',gridGap:'1em',fontSize:'13px'}}>
            ...
            {data.type !== 'Картинка' && data.type !== 'Товар'?JSON.parse(data.obj || "{}").background && JSON.parse(data.obj || "{}").background.length > 4 &&<div style={{position:'absolute',right:'0',top:'0',width:'100%',overflow:'hidden',height:'100%',zIndex:'0'}}>
                {JSON.parse(data.obj || "{}").background.includes('mp4') ?
                    <video id={'backgroundImage'+data.id} style={{width:'100%'}} src={process.env.REACT_APP_API_URL + JSON.parse(data.obj || "{}").background} loop autoPlay controls={false} muted></video>
                    :
