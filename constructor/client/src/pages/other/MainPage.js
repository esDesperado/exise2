...
const MainPage = observer(() => {
    ...
    useEffect(()=>{
        let page1;
        page1 = inface.pages.filter(page=>page.path === document.location.pathname.replace(ADMIN_ROUTE,'').replace('/',''))[0];
    ...
    },[document.getElementById('root').offsetHeight,document.querySelectorAll('.FDAStart').length,(inface.blocks.filter(bl=>bl.page === page.id && bl.type==='post-header')[0]?inface.show:true)])
    ...
    return(
        <div style={{width:'100%',color:inface.interface.color,background:inface.interface.background,fontSize:inface.width > 350?inface.width > 600?'16px':'12px':'11px',position:'relative',}}>
            ...
            <div style={{position:'relative',display:'grid',width:'100%',justifyItems:'center'}}>
                {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE) &&<div style={{zIndex:'1009',position:'relative',display:'grid',justifyItems:'center',gridAutoFlow:'column',alignItems:'center',fontWeight:'bold',width:'100%',textAlign:'center',padding:'5px 15px',color:inface.aback,background:inface.acolor,}}>
                    <div>
