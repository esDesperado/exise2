...
const PostHeader = observer((data,v) => {
    ...
    useEffect(()=>{
        ...
        //document.getElementById('block'+data.id).getBoundingClientRect().width,
    },[h1,w1,data.id,data.obj,document.getElementById('block'+data.id), document.getElementById('block'+data.id) && document.getElementById('block'+data.id).getBoundingClientRect().width, inface.width])
